...
- **High-poly verification:** Run `npm run analyze:gltf -- ./assets/3d/exports/<file>.glb[@scene] --markdown --budget triangles=12000 --budget vertices=6000 --output docs/art/exports/<file>.md` during hero desk integration to confirm mesh bounds track the blockout metrics, inspect instanced vertex/triangle totals against budgets, and capture embeddable logs before committing new renders. The CLI exits with a non-zero status when any budget is exceeded so CI can block heavy exports.
